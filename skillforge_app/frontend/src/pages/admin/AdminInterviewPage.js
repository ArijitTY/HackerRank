...
function TestModal({ onClose, onSaved, editTest }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/admin/interview-tests/parse', { text });
  ...
  const save = async () => {
    ...
    try {
      if (editTest) {
        await api.put(`/admin/interview-tests/${editTest.id}`, { name, description, questions: valid });
      } else {
        await api.post('/admin/interview-tests', { name, description, extractedText, questions: valid });
      }
      onSaved();
    } catch (e) {
...
function AssignModal({ test, onClose }) {
  ...
  useEffect(() => {
    api.get('/admin/candidates').then(r => setCandidates(r.data.candidates || []));
  }, []);
  ...
  const assign = async () => {
    ...
    setAssigning(true);
    try {
      const r = await api.post(`/admin/interview-tests/${test.id}/assign`, { candidateIds: [...selected] });
...
function ReviewModal({ sessionId, onClose, onApproved }) {
  ...
  const load = useCallback(() => {
    api.get(`/admin/interview-results/${sessionId}`).then(r => {
      setData(r.data);
      const ov = {}, nt = {};
  ...
  const runEvaluation = async () => {
    ...
    try {
      const r = await api.post(`/admin/interview-results/${sessionId}/evaluate`);
  ...
  const saveOverride = async (answerId) => {
    ...
    try {
      await api.put(`/admin/interview-results/${sessionId}/answers/${answerId}`, {
  ...
  const approve = async () => {
    setApproving(true);
    try {
      await api.post(`/admin/interview-results/${sessionId}/approve`, { adminNotes });
      onApproved();
    } catch (e) {
...
export default function InterviewPage() {
  ...
  const loadTests = useCallback(() => {
    ...
    api.get('/admin/tests').then(r => setPrepTests((r.data.tests || []).filter(t => t.is_interview_prep === 1))).catch(() => {});
  }, []);

  const loadResults = useCallback(() => {
    api.get('/admin/interview-results').then(r => setResults(r.data.sessions || []));
  }, []);

  useEffect(() => { loadTests(); loadResults(); }, [loadTests, loadResults]);

  const openEdit = async (test) => {
    const r = await api.get(`/admin/interview-tests/${test.id}`);
    setEditTest(r.data);
  };
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 14 }}>
              <button style={S.btn('rgba(255,255,255,0.06)', 'rgba(255,255,255,0.7)')} onClick={() => {
                ...
                fetch('/api/admin/interview-results/export', { headers: { Authorization: `Bearer ${token}` } })

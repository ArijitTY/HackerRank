    ...
    <div style={STAT_CARD}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: color + '22', color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, position: 'relative' }}>
          {icon}
          {pulse && <span style={{ position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: '50%', background: color, animation: 'sf-pulse 1.8s infinite' }} />}
        </div>
      </div>
...
export default function AdminDashboard() {
  ...
  const fetchAll = () => {
    api.get('/admin/dashboard').then(r => setData(r.data)).catch(e => setError(e.response?.data?.error || 'Failed to load dashboard'));
    api.get('/admin/candidates').then(r => setCandidates(r.data.candidates || r.data || [])).catch(() => {});
    api.get('/admin/candidates/online-status').then(r => setOnlineStatus(r.data || {})).catch(() => {});
  };
  ...
  const stats = [
    ...
    { icon: '📋', color: '#3b82f6', value: data.testsAssigned ?? data.tests ?? 0, label: 'Tests' },
    ...
    { icon: '📊', color: '#f59e0b', value: `${data.passRate || 0}%`, label: 'Pass Rate' },
  ];
    ...
    <div className="page-enter">
      ...
      <div style={{ ...CARD_BASE, marginTop: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <QuickAction icon="➕" color="#a78bfa" title="Add Candidate" subtitle="Create a new candidate account" onClick={() => navigate('/admin/candidates')} />
          <QuickAction icon="🧩" color="#14b8a6" title="Design Test" subtitle="Build a custom test" onClick={() => navigate('/admin/design-test')} />
          <QuickAction icon="📈" color="#3b82f6" title="View Results" subtitle="Review submitted sessions" onClick={() => navigate('/admin/results')} />
          <QuickAction icon="🔴" color="#f59e0b" title="Live Monitor" subtitle="Watch in-progress tests" onClick={() => navigate('/admin/live')} />
        </div>
      </div>
    </div>
  );
